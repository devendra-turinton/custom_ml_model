...
class BasePipeline(ABC):
    ...
    def __init__(
        ...
        logger_name = f"training.{self.model_id}.{self.version}"

        # Use provided job_logger or create a new one
        if job_logger:
            self.job_logger = job_logger
        else:
            self.job_logger = logging.getLogger(logger_name)
            if not self.job_logger.handlers:
                # Set up a new file handler if needed
                file_handler = logging.FileHandler(log_file)
                formatter = logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
                file_handler.setFormatter(formatter)
                self.job_logger.setLevel(logging.DEBUG)
                self.job_logger.addHandler(file_handler)
                self.job_logger.propagate = False  # Prevent duplicate logs
        
        # NOW we can start logging
        self.job_logger.info("Initializing pipeline...")

        self.job_logger.debug(f"Using output directory: {self.output_dir}")
        
        # Load configuration
        self.config = ML_CONFIG
        self.job_logger.debug("Using main application configuration")
        ...
        self.job_logger.debug(f"Train-test split parameters: test_size={self.test_size}, random_state={self.random_state}")
        ...
        self.job_logger.info(f"{self.__class__.__name__} initialized - ID: {self.model_id}, Version: {self.version}, Output: {self.output_dir}")
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        """
        self.job_logger.info("Loading data...")
        start_time = datetime.now()
        
        if self.df is not None:
            self.job_logger.info("Using provided DataFrame")
            self.metadata['data']['source'] = 'provided_dataframe'
            self.metadata['data']['shape'] = self.df.shape
            
            # Log data summary information
            self.job_logger.debug(f"Data summary:\n{self._get_data_summary(self.df)}")
            
            load_time = (datetime.now() - start_time).total_seconds()
            self.job_logger.info(f"Using provided DataFrame (rows: {self.df.shape[0]:,}, columns: {self.df.shape[1]:,}) in {load_time:.2f} seconds")
        ...
        try:
            self.job_logger.info(f"Loading data from {self.data_path} (format: {file_ext})")
            
            if file_ext == '.csv':
                self.job_logger.info(f"Loading CSV file: {self.data_path}")
                self.df = pd.read_csv(self.data_path)
                self.job_logger.debug(f"CSV file loaded successfully with {self.df.shape[0]:,} rows and {self.df.shape[1]:,} columns")
            elif file_ext in ['.xls', '.xlsx']:
                self.job_logger.info(f"Loading Excel file: {self.data_path}")
                self.df = pd.read_excel(self.data_path)
                self.job_logger.debug(f"Excel file loaded successfully with {self.df.shape[0]:,} rows and {self.df.shape[1]:,} columns")
            elif file_ext == '.json':
                self.job_logger.info(f"Loading JSON file: {self.data_path}")
                self.df = pd.read_json(self.data_path)
                self.job_logger.debug(f"JSON file loaded successfully with {self.df.shape[0]:,} rows and {self.df.shape[1]:,} columns")
            elif file_ext == '.parquet':
                self.job_logger.info(f"Loading Parquet file: {self.data_path}")
                self.df = pd.read_parquet(self.data_path)
                self.job_logger.debug(f"Parquet file loaded successfully with {self.df.shape[0]:,} rows and {self.df.shape[1]:,} columns")
            else:
            ...
            load_time = (datetime.now() - start_time).total_seconds()
            self.job_logger.info(f"Data loaded in {load_time:.2f} seconds")
            
            # Calculate and log memory usage
            memory_usage_mb = self.df.memory_usage(deep=True).sum() / (1024 * 1024)
            self.job_logger.info(f"DataFrame memory usage: {memory_usage_mb:.2f} MB")
            
            # Log data shape and sample
            self.job_logger.info(f"Data shape: {self.df.shape[0]:,} rows, {self.df.shape[1]:,} columns")
            self.job_logger.debug(f"Data head (first 5 rows):\n{self.df.head().to_string()}")
            ...
            if len(numeric_columns) > 0:
                ...
                self.job_logger.debug(f"Numeric column statistics:\n{self.df[numeric_columns].describe().to_string()}")
            ...
            if columns_with_missing:
                self.job_logger.info(f"Found {len(columns_with_missing)} columns with missing values")
                for col in columns_with_missing:
                    self.job_logger.info(f"  - Column '{col}': {missing_values[col]:,} missing values ({missing_values_percent[col]:.2f}%)")
            else:
    ...
    def detect_problem_type(self) -> str:
        ...
        """
        self.job_logger.info("Detecting problem type...")
        start_time = datetime.now()
        ...
        try:
            self.job_logger.info(f"Examining target column: '{self.target}'")
            ...
            if self.target in self.df.columns:
                ...
                self.job_logger.debug(f"Target column properties - dtype: {target_dtype}, unique values: {unique_values}, is_numeric: {is_numeric}")
                self.job_logger.debug(f"Target value distribution:\n{self.df[self.target].value_counts().head(10).to_string()}")
                
                if is_numeric:
                    self.job_logger.debug(f"Target numeric statistics:\n{self.df[self.target].describe().to_string()}")
            else:
                self.job_logger.error(f"Target column '{self.target}' not found in dataframe")
            ...
            self.job_logger.info(f"Detected problem type: {self.problem_type} in {detection_time:.2f} seconds")
            ...
            if self.problem_type == 'classification':
                ...
                self.job_logger.info(f"Class distribution in target '{self.target}':")
                for cls, pct in class_distribution.items():
                    self.job_logger.info(f"  - {cls}: {pct:.2f}%")
                
                # Check class imbalance
                if class_distribution.min() < 10:  # Arbitrary threshold of 10%
                    self.job_logger.warning(f"Class imbalance detected: Minority class represents only {class_distribution.min():.2f}% of the data")
                    
            elif self.problem_type == 'regression':
                # For regression, log distribution statistics
                target_stats = self.df[self.target].describe()
                self.job_logger.info(f"Target '{self.target}' regression statistics:")
                self.job_logger.info(f"  - Range: {target_stats['min']:.4f} to {target_stats['max']:.4f}")
                self.job_logger.info(f"  - Mean: {target_stats['mean']:.4f}, Std: {target_stats['std']:.4f}")
                self.job_logger.info(f"  - Quartiles: 25%={target_stats['25%']:.4f}, 50%={target_stats['50%']:.4f}, 75%={target_stats['75%']:.4f}")
    ...
    def preprocess_data(self, custom_feature_engineering: Optional[Callable] = None) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
        ...
        """
        self.job_logger.info("Preprocessing data...")
        overall_start_time = datetime.now()
        
        preprocessing_metadata = {}
        
        # Split into features and target
        self.job_logger.debug("Splitting data into features and target")
        start_time = datetime.now()
        X = self.df.drop(columns=[self.target])
        y = self.df[self.target]
        split_time = (datetime.now() - start_time).total_seconds()
        self.job_logger.debug(f"Split data into features and target in {split_time:.2f} seconds")
        
        self.job_logger.info(f"Features shape: {X.shape}, Target shape: {y.shape}")
        ...
        if custom_feature_engineering:
            self.job_logger.info("Applying custom feature engineering...")
            start_time = datetime.now()
            try:
                X_before = X.shape
                ...
                self.job_logger.info(f"Custom feature engineering completed in {engineering_time:.2f} seconds")
                self.job_logger.info(f"Features shape before engineering: {X_before}, after: {X_after}")
                
                # Log new features if any were added
                if X_after[1] > X_before[1]:
                    new_features = set(X.columns) - set(self.df.columns)
                    self.job_logger.info(f"Added {len(new_features)} new features: {', '.join(new_features)}")
                ...
                self.job_logger.error(error_msg, exc_info=True)
                self.job_logger.warning("Continuing with original features")
                preprocessing_metadata['custom_features_applied'] = False
                preprocessing_metadata['custom_features_error'] = str(e)
        
        # Identify timestamp columns
        self.job_logger.debug("Identifying timestamp columns")
        start_time = datetime.now()
        timestamp_columns = []
        for col in X.columns:
            if pd.api.types.is_datetime64_any_dtype(X[col]):
                timestamp_columns.append(col)
                self.job_logger.debug(f"Found datetime column: {col}")
            # Try to convert string columns that might be timestamps
            elif X[col].dtype == 'object':
                try:
                    sample = X[col].dropna().iloc[0] if not X[col].dropna().empty else None
                    if sample:
                        self.job_logger.debug(f"Attempting to convert column '{col}' to datetime. Sample value: {sample}")
                    pd.to_datetime(X[col], errors='raise')
                    # Convert to actual datetime
                    X[col] = pd.to_datetime(X[col])
                    timestamp_columns.append(col)
                    self.job_logger.debug(f"Successfully converted '{col}' to datetime")
                except Exception as e:
                    self.job_logger.debug(f"Column '{col}' is not a datetime: {str(e)}")
                    pass
        
        timestamp_detection_time = (datetime.now() - start_time).total_seconds()
        self.job_logger.info(f"Identified {len(timestamp_columns)} timestamp columns in {timestamp_detection_time:.2f} seconds")
        if timestamp_columns:
            self.job_logger.info(f"Timestamp columns: {', '.join(timestamp_columns)}")
        ...
        if self.problem_type == 'classification' and self.config.get('common', {}).get('train_test_split', {}).get('stratify', True):
            stratify = y
            self.job_logger.info("Using stratified sampling for train-test split")
        
        # Split into train and test sets
        self.job_logger.info(f"Splitting data with test_size={self.test_size}, random_state={self.random_state}")
        start_time = datetime.now()
        ...
        self.job_logger.info(f"Train-test split completed in {split_time:.2f} seconds")
        self.job_logger.info(f"Train set size: {self.X_train.shape[0]:,} rows, Test set size: {self.X_test.shape[0]:,} rows")
        ...
        if self.problem_type == 'classification':
            ...
            self.job_logger.info("Class distribution in train set:")
            for cls, pct in train_class_dist.items():
                self.job_logger.info(f"  - {cls}: {pct:.2f}%")
            
            self.job_logger.info("Class distribution in test set:")
        ...
        self.job_logger.info("Creating preprocessing pipeline...")
        ...
        self.job_logger.debug(f"Preprocessing configuration:")
        self.job_logger.debug(f"  - Missing values strategies: numeric={missing_values_config.get('numeric_strategy', 'median')}, categorical={missing_values_config.get('categorical_strategy', 'most_frequent')}")
        self.job_logger.debug(f"  - Outlier handling: method={outlier_config.get('method', 'iqr')}, threshold={outlier_config.get('threshold', 1.5)}, strategy={outlier_config.get('strategy', 'clip')}")
        self.job_logger.debug(f"  - Scaling method: {feature_eng_config.get('scaling', 'standard')}")
        
        # Numeric transformer pipeline
        start_time = datetime.now()
        self.job_logger.debug("Creating numeric transformation pipeline")
        ...
        if scaling_method != 'none':
            if scaling_method == 'standard':
                ...
                self.job_logger.debug("Added StandardScaler to numeric pipeline")
            elif scaling_method == 'minmax':
                numeric_transformer.steps.append(('scaler', MinMaxScaler()))
                self.job_logger.debug("Added MinMaxScaler to numeric pipeline")
        
        numeric_pipeline_time = (datetime.now() - start_time).total_seconds()
        self.job_logger.debug(f"Created numeric transformation pipeline in {numeric_pipeline_time:.2f} seconds")
        
        # Categorical transformer
        start_time = datetime.now()
        self.job_logger.debug("Creating categorical transformation pipeline")
        ...
        self.job_logger.debug(f"Created categorical transformation pipeline in {categorical_pipeline_time:.2f} seconds")
        
        # Identify column types
        self.job_logger.debug("Identifying column types")
        start_time = datetime.now()
        ...
        self.job_logger.info(f"Identified column types in {column_type_time:.2f} seconds:")
        self.job_logger.info(f"  - Numeric features: {len(numeric_features):,}")
        self.job_logger.info(f"  - Categorical features: {len(categorical_features):,}")
        
        # Log some sample column names of each type
        if numeric_features:
            self.job_logger.debug(f"Sample numeric features (max 5): {', '.join(numeric_features[:5])}")
        if categorical_features:
            self.job_logger.debug(f"Sample categorical features (max 5): {', '.join(categorical_features[:5])}")
        ...
        self.job_logger.debug("Creating complete preprocessing pipeline")
        start_time = datetime.now()
        ...
        self.job_logger.debug(f"Created complete preprocessor in {preprocessor_creation_time:.2f} seconds")
        
        # Fit preprocessor on training data
        self.job_logger.info("Fitting preprocessor on training data...")
        start_time = datetime.now()
        try:
            self.preprocessor.fit(self.X_train)
            fit_time = (datetime.now() - start_time).total_seconds()
            self.job_logger.info(f"Preprocessor fitted successfully in {fit_time:.2f} seconds")
            
            # Log transformed feature names if available
            try:
                feature_names_out = self.preprocessor.get_feature_names_out()
                self.job_logger.debug(f"Transformed feature count: {len(feature_names_out)}")
                if len(feature_names_out) <= 20:  # Only log all names if not too many
                    self.job_logger.debug(f"Transformed feature names: {', '.join(feature_names_out)}")
                else:
                    self.job_logger.debug(f"First 10 transformed feature names: {', '.join(feature_names_out[:10])}")
                
                preprocessing_metadata['transformed_feature_count'] = len(feature_names_out)
            except Exception as e:
                self.job_logger.debug(f"Could not extract transformed feature names: {str(e)}")
            ...
            if categorical_transformer_idx is not None and categorical_features:
                try:
                    ...
                    self.job_logger.info(f"One-hot encoding expanded {len(categorical_features)} categorical features into {total_encoded_features} binary features")
                    preprocessing_metadata['one_hot_encoding'] = {
                        'input_features': len(categorical_features),
                        'output_features': total_encoded_features
                    }
                except Exception as e:
                    self.job_logger.debug(f"Could not extract one-hot encoding details: {str(e)}")
    ...
    def save_model(self) -> str:
        ...
        self.job_logger.info(f"Saving best model to {self.output_dir}")
        start_time = datetime.now()
        ...
        try:
            ...
            self.job_logger.info(f"Model size: {model_size_mb:.2f} MB")
        except Exception as e:
            self.job_logger.debug(f"Could not determine model size: {str(e)}")
        ...
        try:
            ...
            if os.path.exists(model_filename):
                ...
                self.job_logger.info(f"Saved model file size: {file_size_mb:.2f} MB")
            else:
                self.job_logger.warning(f"Model file not found after saving: {model_filename}")
    ...
    def save_metadata(self) -> str:
        ...
        """
        self.job_logger.info("Saving metadata...")
        start_time = datetime.now()
        ...
        if len(predictions_samples) > 1000:
            self.job_logger.info(f"Large number of prediction samples detected ({len(predictions_samples)}), saving to separate file")
            ...
            self.job_logger.info(f"Prediction samples saved to {predictions_file}")
        ...
        save_time = (datetime.now() - start_time).total_seconds()
        self.job_logger.info(f"Metadata saved to {metadata_file} in {save_time:.2f} seconds")
    ...
    def run_pipeline(self) -> Tuple[Any, pd.DataFrame]:
        ...
        """
        self.job_logger.info(f"Starting ML pipeline run - ID: {self.model_id}, Version: {self.version}")
        ...
        try:
            ...
            self.job_logger.info(f"System resources at pipeline start: CPU {cpu_percent}%, RAM {memory_info.percent}% (Available: {memory_info.available / (1024**3):.2f} GB)")
        except ImportError:
            self.job_logger.debug("psutil not available for resource monitoring")
        ...
        try:
            ...
            step_time = (datetime.now() - step_start).total_seconds()
            self.job_logger.info(f"Train set: {self.X_train.shape[0]:,} rows, Test set: {self.X_test.shape[0]:,} rows")
            self.job_logger.info(f"Step completed in {step_time:.2f} seconds")
            ...
            self.train_models()
            step_time = (datetime.now() - step_start).total_seconds()
            self.job_logger.info(f"Trained {len(self.models):,} models")
            self.job_logger.info(f"Step completed in {step_time:.2f} seconds")
            ...
            try:
                ...
                memory_info = psutil.virtual_memory()
                cpu_percent = psutil.cpu_percent(interval=0.1)
                self.job_logger.info(f"System resources at pipeline end: CPU {cpu_percent}%, RAM {memory_info.percent}% (Available: {memory_info.available / (1024**3):.2f} GB)")
            except ImportError:
                self.job_logger.debug("psutil not available for resource monitoring")
            ...
            if hasattr(self, 'job_logger') and self.job_logger:
                for handler in list(self.job_logger.handlers):
                    self.job_logger.removeHandler(handler)
                    handler.close()
                self.job_logger.debug(f"Cleaned up job-specific logger handlers for {self.model_id}.{self.version}")
            ...
            self.job_logger.debug(f"Exception traceback:\n{error_details}")
            ...
            try:
                ...
                memory_info = psutil.virtual_memory()
                cpu_percent = psutil.cpu_percent(interval=0.1)
                self.job_logger.info(f"System resources at failure: CPU {cpu_percent}%, RAM {memory_info.percent}% (Available: {memory_info.available / (1024**3):.2f} GB)")
            except ImportError:
                self.job_logger.debug("psutil not available for resource monitoring")
            ...
            pipeline_runtime = (datetime.now() - pipeline_start).total_seconds()
            self.job_logger.error(f"Pipeline failed after {pipeline_runtime:.2f} seconds")
