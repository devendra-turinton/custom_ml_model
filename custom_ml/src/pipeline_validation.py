...
def create_validation_report(validation_results: Dict[str, Dict[str, bool]], output_dir: str) -> str:
    """
    Create a human-readable validation report and save it to the output directory.
    ...
    """
    report_lines = [
        "# Custom Pipeline Validation Report",
    ...
    for section, checks in validation_results.items():
        ...
        status = "✅ PASSED" if section_validities[section] else "❌ FAILED"
        report_lines.append(f"- {section}: {passed_count}/{total_count} checks passed {status}")
    
    all_passed = all(section_validities.values())
    report_lines.append("")
    report_lines.append(f"Overall validation: {'✅ PASSED' if all_passed else '❌ FAILED'}")
    report_lines.append("")
    ...
    for section, checks in validation_results.items():
        ...
        for check, passed in checks.items():
            ...
            readable_check = check.replace("_", " ").capitalize()
            report_lines.append(f"| {readable_check} | {status} |")
    ...
    for section, checks in validation_results.items():
        for check, passed in checks.items():
            if not passed:
                failed_checks.append((section, check))
    
    if failed_checks:
        report_lines.append("## Recommendations")
        report_lines.append("")
        
        for section, check in failed_checks:
            recommendation = get_recommendation_for_check(section, check)
            report_lines.append(f"### {section}: {check}")
...
def get_recommendation_for_check(section: str, check: str) -> str:
    """
    Get a recommendation for fixing a failed validation check.
    
    Args:
        section: The validation section
    ...
    recommendations = {
        "method_validation": {
            "correct_signature": "Ensure your custom pipeline function accepts these parameters: `df`, `target`, `model_id`, `output_dir`, `config`, and `**kwargs`.",
            "docstring_present": "Add a docstring to your custom pipeline function describing its purpose and parameters."
        },
        "return_value_validation": {
            "returns_tuple": "Your function should return a tuple with at least 4 elements.",
            "has_four_elements": "Your function should return a tuple with at least 4 elements: (best_model, results_df, model_path, metadata_path).",
            "has_model": "The first element of the return tuple should be the best model object.",
            "has_results_df": "The second element of the return tuple should be a pandas DataFrame with model results.",
            "has_model_path": "The third element of the return tuple should be a string path to the saved model file.",
            "has_metadata_path": "The fourth element of the return tuple should be a string path to the saved metadata file."
        },
        "file_validation": {
            "model_file_exists": "Ensure the model file is saved to the specified path.",
            "model_file_not_empty": "The model file exists but is empty. Check your model saving code.",
            "model_file_loadable": "The model file cannot be loaded with pickle. Ensure you're saving it correctly.",
            "metadata_file_exists": "Ensure the metadata file is saved to the specified path.",
            "metadata_file_not_empty": "The metadata file exists but is empty. Check your metadata saving code.",
            "metadata_parsable": "The metadata file cannot be parsed as JSON. Check for syntax errors.",
            "log_file_exists": "Ensure your custom pipeline is writing logs to the output directory.",
            "log_file_not_empty": "The log file exists but is empty. Check your logging configuration."
        },
        "metadata_validation": {
            "required_fields_present": "Your metadata must include these fields: 'timestamp', 'model_id', 'target', 'status'.",
            "best_model_info_present": "Your metadata should include a 'best_model' section with at least a 'name' field.",
            "status_completed": "Your metadata 'status' field should be set to 'completed' when successful.",
            "test_metrics_present": "Your metadata should include test metrics in the 'best_model.metrics' section.",
            "preprocessing_info_present": "Your metadata should include preprocessing information such as 'numeric_features' and 'categorical_features'.",
            "problem_type_present": "Your metadata MUST include 'problem_type' in either parameters or best_model section for testing.",
            "target_column_specified": "Your metadata MUST specify the target column for testing.",
            "feature_types_defined": "Your metadata MUST include feature type information in the preprocessing section for testing."
        }
    }
    
    return recommendations.get(section, {}).get(check, "No specific recommendation available. Review the check name for guidance.")